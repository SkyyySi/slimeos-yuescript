...
// (surface: cairo.Surface, width: integer, height: integer) -> Gdk.Pixbuf
...
GdkPixbuf* blur_pixbuf(GdkPixbuf* pixbuf, const i32 radius, const bool no_use_kernel) {
	...
	/*
	printf("--> Kernel: \n");
	for (i64 y_loop = 0; y_loop < radius; y_loop++) {
		printf("[");
		for (i64 x_loop = 0; x_loop < radius; x_loop++) {
			printf("%lf, ", (kernel[x_loop + (y_loop * radius)]));
		}
		printf("]\n");
	}
	//*/
...
static const struct luaL_Reg c_filter[] = {
...
int luaopen_c_filter(lua_State* L) {
	luaL_newlib(L, c_filter);
